...
from api1.models import storeFloorPlan,cartManager,storeFixtures,cameraService,cameraInfo,groundplot# Locationsetup,Groundplot,Camera_gpu,store_Dimension,Shelves,Master_Host,Cart_Manager,Backend_APIs
from django.views.decorators.csrf import csrf_exempt
from rest_framework.decorators import api_view
from rest_framework import status
from api1.serializers import storeFixturesserializers,cartManagerserializers,storeFloorPlanserializers,cameraServiceserializers,cameraInfoserializers,groundplotserializers# Locationsetupserializers,store_Dimensionserializers,Shelvesserializers,Master_Hostserializers,Camera_gpuserializers,Groundplotserializers,Cart_Managerserializers,Backend_APIsserializers
...
def all_(request):
    ...
    if  request.method == "GET":
        try:
            ...
            try:
                ground = groundplot.objects.filter(locationId=tutorial_data['locationId']).values()
                if ground:
                    a.update({"groundplot":ground[0]})
            except:
                pass
            #print("222222222222222222")
            store = storeFloorPlan.objects.filter(locationId=tutorial_data['locationId']).values()
            if store:
            ...
            she = storeFixtures.objects.filter(locationId=tutorial_data['locationId']).values()
            if she:
...
def storeFloorPlan_(request):
    if request.method == "PUT":
        ...
        a = storeFloorPlan.objects.filter(locationId=tutorial_data['locationId']).values()
        #b = a[0]
        if a:
            location = storeFloorPlan.objects.get(locationId=tutorial_data['locationId'])
            tutorial_serializer = storeFloorPlanserializers(location, data=tutorial_data)
...
def storeFixtures_(request):
    if request.method == "PUT":
        tutorial_data = JSONParser().parse(request)
        a = storeFixtures.objects.filter(locationId=tutorial_data['locationId']).values()
        #b = a[0]
        if a:
...
def cameraService_(request):
    if  request.method == "PUT":
        tutorial_data = JSONParser().parse(request)
        a = cameraService.objects.filter(locationId=tutorial_data['locationId']).values()
        aa = json.loads(json_util.dumps(tutorial_data))#b = a[0]
        if a:
